...
class MajPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('maj.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `maj`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM maj WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO maj (date, title, link, content)"
